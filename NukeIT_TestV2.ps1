...
param (
    ...
    [Parameter(Mandatory=$false)]
    [int]$ProfileAge = 150 # Define number of days profile has been inactive
)

$AppName = "NukeIT"

# Logging: (AppName variable + .log)
...
function Clean-System {
    ...
    # Configure Disk Cleanup (Set Reg flags for removeal of corrospondig directories. This is a more streamlined method.)
    ...
    $cleanupItems = @(
        ...
        "Memory Dump Files", #Defined here as well.
        "Old ChkDsk Files",
    ...
    $cleanupItems | ForEach-Object {
        Set-ItemProperty -Path "$CleanMgrKey\$_" -Name StateFlags0001 -Type DWORD -Value 2
    }

    # Clean Additional Directories (Taken from Alan's Original Script)
    ...
    try {
        ...
        if ($Computer -eq $env:ComputerName) {
            ...
            Invoke-Command -ComputerName $Computer -ScriptBlock {
                ...
                Clean-System -ProfileAge $ProfileAge -CleanCCMCache:$CleanCCMCache
            } -ArgumentList $ProfileAge, $CleanCCMCache
        }
    }
    catch {
